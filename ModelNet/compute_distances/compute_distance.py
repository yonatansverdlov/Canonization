...
#   score = REDUCE_{X in test} min_{Y in train, same class} d(X,Y)
#
# REDUCE modes:
#   average : mean over test set
#   max     : max over test set
#
# Metrics:
...
DatasetReduce = Literal["average", "max"]
...
class CloudDistanceConfig:
    ...
    dataset_reduce: DatasetReduce = "average"
...
def wass_hungarian_batch(X: torch.Tensor, Y: torch.Tensor) -> torch.Tensor:
    ...
    for b in range(B):
        ...
        out[b] = C[b, row_ind, col_ind].mean()
    return out


# -------------------------
# Dataset reduction helper
# -------------------------
def reduce_over_dataset(vals: List[float], mode: DatasetReduce) -> float:
    if len(vals) == 0:
        raise ValueError("Cannot reduce empty list of dataset values")
    ...
    raise ValueError(f"Unknown dataset_reduce mode: {mode}")
...
def score_perm_metric(
    ...
    return reduce_over_dataset(vals, cfg.dataset_reduce)
...
def score_wasserstein(
    ...
    for t_idx, td in enumerate(test_ds):
        ...
        for s in range(0, N, wass_batch):
            ...
            d_batch = wass_hungarian_batch(X, Y)
            bmin = d_batch.min()
            best = bmin if best is None else torch.minimum(best, bmin)
